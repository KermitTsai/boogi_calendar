...
public class EventAdapter_af_to extends RecyclerView.Adapter<EventAdapter_af_to.EventViewHolder>{
    List<Event_af_to> mListEvents;
    private ViewBinderHelper viewBinderHelper =new ViewBinderHelper();

    String[] color ={"red","orange","yellow","teal","green","blue","navy","light_purple"};

    public EventAdapter_af_to(List<Event_af_to> mListEvents) {
    ...
    public void onBindViewHolder(@NonNull EventViewHolder holder, int position) {
        Event_af_to event = mListEvents.get(position);
        if(event ==null){
            return;
        }
        ...
        switch (event.getKind()){
            ...
            case "Schedule":
                ...
                switch(event.getColor()){
                    case "yellow":
                        holder.ivEventKind.setColorFilter(Color.parseColor("#FFB703"));
                        holder.vColor.setBackgroundColor(Color.parseColor("#FFB703"));
                        break;
                    case "green":
                        holder.ivEventKind.setColorFilter(Color.parseColor("#4CAF50"));
                        holder.vColor.setBackgroundColor(Color.parseColor("#4CAF50"));
